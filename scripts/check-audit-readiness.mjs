...
for (const required of [
  ...
  assert(auditReadme.includes(required), `docs/audit/README.md must mention ${required}`);
}

const manifestFormat = read('docs/audit/MANIFEST_FORMAT.md');
for (const required of [
  'AUDITED_ERGOTREES.json',
  'AUDITED_CONTRACTS.json',
  'mainnetAllowed',
  'signed external audit',
]) {
  assert(manifestFormat.includes(required), `MANIFEST_FORMAT.md must mention ${required}`);
}
