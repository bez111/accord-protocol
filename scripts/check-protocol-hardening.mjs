...
function listMarkdownFiles(dir) {
  ...
  for (const name of fs.readdirSync(path.join(root, dir))) {
    if (['.git', 'dist', 'node_modules', 'site'].includes(name)) continue;
    const rel = path.join(dir, name);
    const abs = path.join(root, rel);
    const stat = fs.statSync(abs);
    if (stat.isDirectory()) out.push(...listMarkdownFiles(rel));
    else if (name.endsWith('.md')) out.push(rel);
  }
  return out;
}

for (const name of fs.readdirSync(path.join(root, 'docs/audit'))) {
  const rel = path.join('docs/audit', name);
  assert(
    publicAuditFiles.has(rel),
...
const publicMarkdownFiles = new Set([
  ...
  ...listMarkdownFiles('specs'),
  ...listMarkdownFiles('packages'),
]);

for (const rel of publicMarkdownFiles) {
  const text = readText(rel);
  for (const match of text.matchAll(/docs\/audit\/([A-Za-z0-9_.-]+\.md)/g)) {
    const auditRel = `docs/audit/${match[1]}`;
    assert(
      publicAuditFiles.has(auditRel),
      `${rel} must not link to non-public audit material ${auditRel}`,
    );
  }
}
