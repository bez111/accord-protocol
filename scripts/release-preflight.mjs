...
//
// Pre-flight smoke for v0.4.2 release. Runs every gate the publish workflow
...
//   3. Version distribution (10 × 0.4.2 + 8 × 0.3.2 expected)
...
// Reference rail packages at 0.3.2. The @accord-protocol/* tarballs declare
// runtime deps on these (e.g. rails-base depends on agentpay-base@^0.3.2), so
// the install-in-tempdir gate has to ship them as local tarballs too — the
// 0.3.2 line is not yet on the public npm registry.
...
gate("03 version distribution (10×0.4.2 + 8×0.3.2)", () => {
  ...
  const expect040 = counts.get("0.4.2") ?? 0;
  const expect030 = counts.get("0.3.2") ?? 0;
  if (expect040 !== 10 || expect030 !== 8) {
    return fail(
      `expected 10×0.4.2 + 8×0.3.2; got ${[...counts].map(([v, c]) => `${c}×${v}`).join(" + ")}`,
    );
  }
  return pass(`10×0.4.2 (Accord) + 8×0.3.2 (legacy)`);
});
...
if (RUN_PACK) {
  ...
  gate("18 install-in-tempdir smoke for all 18 workspace packages", () => {
    ...
    // Build name → tarball-path map so we can resolve transitive 0.3.2 deps
    // (e.g. rails-base depends on agentpay-base@^0.3.2) against the local
    // tarballs via npm `overrides`, since the 0.3.2 packages are not on the
    // public registry yet.
    const nameToTarball = new Map();
    for (const file of allTarballs) {
      const full = path.join(PACK_TARBALL_DIR, file);
      // Parse name out of the tarball filename: scoped accord-protocol-core-0.4.2.tgz
      // becomes @accord-protocol/core; legacy ergo-agent-pay-0.3.2.tgz stays as-is.
      let name;
    ...
    try {
      ...
      // npm to resolve transitive 0.3.2 constraints (e.g. rails-base →
      // agentpay-base@^0.3.2) against the local tarballs rather than the
      // public registry.
      const dependencies = {};
...
console.log(`Accord Protocol v0.4.2 release pre-flight\n`);
let failures = 0;
...
if (failures === 0) {
  ...
  const nextStep = branch === "main"
    ? "Ready to tag v0.4.2."
    : "Branch pre-flight passed; merge to main before tagging v0.4.2.";
