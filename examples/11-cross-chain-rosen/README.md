...
| [`ergo-agent-pay`](../../packages/ergo-agent-pay) | SDK both sides drive. Audit gate enforces `basis_token_reserve_v0` on mainnet. |
| [`ergo-agent-scripts`](../../packages/ergo-agent-scripts) | Compiled `basis_token_reserve_v0` ergoTree from the audited manifest. |
...
* The Rosen mainnet TokenMap JSON (from `@rosen-bridge/tokens` or
  Rosen ops) — passed via `ROSEN_TOKEN_MAP_PATH`
...
npm install
SELLER_ADDRESS=9X... \
ROSEN_TOKEN_MAP_PATH=./rosen-mainnet-tokens.json \
node server.ts
