...
For ChainCash (Ergo), the Reserve script enforces rail-specific invariants. Production use requires signed external audit evidence for the exact compiled artifact.
...
| Name | What it checks | Mainnet status |
|---|---|---|
| `task_hash_v0` | Hash-based task-output predicate | Testnet/demo only unless future signed audit evidence explicitly approves it |
| `credential_v0` | Credential-bound task-output predicate | Candidate only after signed external audit evidence |
| `chaincash_*_v0` (reserve / receipt / note) | Full ChainCash semantics — see source | Candidate only after signed external audit evidence |

Conformant implementations MUST NOT treat any predicate as production-mainnet certified unless the relevant manifest is signed and the exact artifact is marked `mainnetAllowed: true`.
