...
Use the high-level `ErgoAgentPay` methods so production safety checks run
before any transaction is signed:

```typescript
await agent.createReserve(...)
await agent.issueNote(...)
await agent.redeemNote(...)
await agent.batchSettle(...)
await agent.deployTracker(...)
```

Raw lifecycle builders are internal plumbing and are not exported from the
package root.
