...
| P4 Testnet pilots | Started | pilot matrix, result template, testnet wallet setup, per-rail rollback plans, mock pilot pass, Sage Ergo testnet settlement evidence record, Rosen architecture stub pass, x402 local stub pilot pass, Base local contract-stub runner, Base Sepolia live runner, machine-checked completed/pending pilot status, `npm run pilots:todo`, and Ergo testnet env preflight committed; Base external evidence still pending |
...
- execute the remaining Base P4 pilot runbook against live Base Sepolia and archive a dated result record when external testnet credentials are available;
