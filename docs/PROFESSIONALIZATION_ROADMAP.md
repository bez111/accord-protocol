...
| P4 Testnet pilots | Started | pilot matrix, result template, testnet wallet setup, per-rail rollback plans, mock pilot pass, Sage Ergo testnet settlement evidence record, machine-checked completed/pending pilot status, `npm run pilots:todo`, and Ergo testnet env preflight committed; Rosen/Base/x402 external evidence still pending |
