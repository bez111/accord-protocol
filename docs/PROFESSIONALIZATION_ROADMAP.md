...
the standard place where agents can agree on work, authorize payment through any
compatible rail, verify completion, emit signed receipts, pass conformance, and
remain legible to wallets, providers, registries, and auditors.

The full strategic frame lives in
[`docs/STRATEGIC_NORTH_STAR.md`](./STRATEGIC_NORTH_STAR.md). The immediate
priority is not adding dozens of new features. It is turning the existing
testnet evidence into trust: launch readiness, audit evidence, provider
onboarding, rail maturity, and then controlled mainnet.
...
As of 2026-05-24, the P0-P3 base, the `v0.4.2` package release, the public
website/learn hub, and Base Sepolia live contract-rail evidence have been
...
|---|---|---|
| P0 Repository stabilization | Complete on `main` | root build/test/typecheck path repaired, release preflight aligned, CJS/path/package data fixes committed, final `npm run release:preflight:pack` passed on `main`, `v0.4.2` is tagged, 18/18 npm package versions are published, and the Python reference package is live on PyPI |
...
| P3 Developer experience | Complete for public review | package matrix, full example-mode matrix, safer legacy/mainnet wording, `noteBoxId` DX, Rosen example cleanup, example 16 CI coverage, contributor templates, public README wording cleanup, release-readiness CI, public website, learn hub, sitemap/RSS, `llms.txt`, `agents.txt`, GitHub repository metadata/topics, and launch drafts committed |
| P4 Testnet pilots | Complete for current matrix | pilot matrix, result template, testnet wallet setup, per-rail rollback plans, mock pilot pass, Sage Ergo testnet settlement evidence record, Sage full receipt live recheck with schema-valid public JSON and L1 conformance, Rosen architecture stub pass, x402 local stub pilot pass, Base local contract-stub runner, Base Sepolia live contract rail pass with external transaction evidence, machine-checked `npm run pilots:check` / `npm run pilots:todo` status at 5/5 complete, and Ergo testnet env preflight committed |
...
- complete the pre-launch polish gates in [`docs/LAUNCH_READINESS.md`](./LAUNCH_READINESS.md) before broad distribution;
- keep the provider onboarding path in [`docs/PROVIDER_ONBOARDING.md`](./PROVIDER_ONBOARDING.md) and [`examples/17-provider-onboarding-kit`](../examples/17-provider-onboarding-kit/) clear enough that a third-party service can become Accord-compatible without private context;
- keep the Sage Ergo, Base Sepolia, x402, Rosen, and mock proof narratives easy to verify from [`docs/RAIL_MATURITY_MATRIX.md`](./RAIL_MATURITY_MATRIX.md), public status, roadmap, pilot records, and learn pages;
- keep Rosen example 11 out of the root workspace until external TokenMap dependencies are suitable for clean CI ([#73](https://github.com/accord-protocol/accord-protocol/issues/73));
- keep turning deterministic pilot failures into tests or tracked issues; `npm run pilots:sage:live` now guards the schema-valid Sage public receipt surface;
- execute the private audit scope, obtain external audit reports, and commit signed manifest updates before any P5 mainnet promotion ([#72](https://github.com/accord-protocol/accord-protocol/issues/72)).

Deferred until the pre-launch gates pass:

- submit `https://accordprotocol.ai/sitemap.xml` through Google Search Console and Bing Webmaster Tools from project-owner accounts ([#70](https://github.com/accord-protocol/accord-protocol/issues/70));
- publish the prepared HN/Reddit/X/Discord/mcp.so launch drafts from project/community accounts and monitor replies for 24-48 hours ([#70](https://github.com/accord-protocol/accord-protocol/issues/70)).
...
- ensure dangerous mainnet override flags fail closed and are absent from examples;
- keep a repeatable auditor handoff bundle outside the public repository.

Acceptance criteria:

- private audit handoff names exact artifacts and expected outputs;
- manifest formats cover tree hashes, bytecode hashes, source revisions, audit status, and `mainnetAllowed`;
- private audit findings are either fixed or tracked as explicit accepted risks;
