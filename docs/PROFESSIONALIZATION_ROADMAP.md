...
| P4 Testnet pilots | Started | pilot matrix, result template, testnet wallet setup, per-rail rollback plans, mock pilot pass, Sage Ergo testnet settlement evidence record, Rosen architecture stub pass, x402 local stub pilot pass, Base local contract-stub runner, machine-checked completed/pending pilot status, `npm run pilots:todo`, and Ergo testnet env preflight committed; Base external evidence still pending |
