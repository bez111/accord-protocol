...
Live Sage receipt recheck:

```bash
npm run pilots:sage:live
```

The latest Sage public surface exposes a full receipt bundle, signed L1
conformance evidence, schema-valid Accord v0 receipt JSON, and live Ergo testnet
settlement evidence. Keep this as an external input because the public URL,
Vercel Blob bundle, signer configuration, and Ergo testnet explorer state live
