...
| Network | `scriptErgoTree` | legacy bypass flag | Behaviour |
|---|---|---|---|
| `testnet` | any | any | always allowed |
| `mainnet` | set (non-empty) | any | passes Gate 1 |
| `mainnet` | missing / empty | `true` | **rejected** with `INSECURE_MAINNET_MODE` |
| `mainnet` | missing / empty | not set / `false` | **rejected** with `INSECURE_MAINNET_MODE` |

Legacy bypass flags are still accepted by older config surfaces, but they no
longer bypass mainnet safety. On mainnet they fail closed; on testnet they are
ignored.
...
| Network | `auditPolicy` verdict | legacy bypass flag | Behaviour |
|---|---|---|---|
...
| `mainnet` | not configured | `true` | **rejected** with `UNAUDITED_ERGOTREE` |
...
3. Add a small wrapper around the SDK that loads your pinned ergoTrees and
   ...
   so Gate 2 fires automatically. Do not depend on legacy dangerous flags;
   they are rejected on mainnet.
...
If you find yourself reaching for a dangerous mainnet bypass because something
else is in the way, treat that as a signal to stop and fix the upstream issue.
...
* [`SECURITY.md`](../SECURITY.md) — public security policy and reporting flow.
