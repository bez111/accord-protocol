...
| `--allow-insecure-dev-mode` | `ERGO_ALLOW_INSECURE_DEV_MODE=1` | `false` | Legacy compatibility flag; no longer bypasses mainnet safety. See [SPEC.md §6](../../SPEC.md). |
...
| `3` | `INSECURE_MAINNET_MODE` — refused to run an unsafe mainnet op |
...
without a compiled `--script`; it will also refuse unaudited scripts unless
an SDK wrapper supplies an external audit policy. The legacy
`--allow-insecure-dev-mode` flag is parsed for compatibility but does not
bypass mainnet safety. On testnet there is no such restriction. See
