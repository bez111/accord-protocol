...
describe("AUDITED_ERGOTREES manifest shape", () => {
  ...
  it("task_hash_v0 notes stay testnet/demo only", () => {
    const e = getAuditedEntry("task_hash_v0");
    assert.match(e.notes, /testnet\/demo only/i);
  });

  it("task_hash_v0 stays mainnetAllowed=false unless a future signed audit approves it", () => {
    const e = getAuditedEntry("task_hash_v0");
    assert.equal(
      e.mainnetAllowed,
      false,
      "task_hash_v0 must stay mainnetAllowed=false before signed external audit evidence",
    );
  });
});
...
describe("verifyAuditedErgoTree", () => {
  ...
  it("requireMainnet rejects task_hash_v0 while mainnetAllowed=false", () => {
    // Re-state the manifest invariant at the SDK boundary: the auditPolicy
    // path used by ergo-agent-pay's `assertProductionSafety` calls
    // verifyAuditedErgoTree under the hood. As long as the manifest entry
    // stays mainnetAllowed=false, the SDK refuses mainnet writes.
