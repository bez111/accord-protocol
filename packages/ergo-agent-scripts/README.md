...
When the SDK runs on mainnet it refuses to issue Notes / create Reserves /
deploy Trackers without a reviewed `scriptErgoTree`. `tryGetErgoTree(name)`
is the canonical way to obtain that value. When the registry is unfilled
`tryGetErgoTree` returns `null`, the SDK refuses to write, and the host gets
a clear error instead of accidentally producing a P2PK box that masquerades
as a predicate-bound one.
