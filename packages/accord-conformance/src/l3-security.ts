...
//     (UNAUDITED_ERGOTREE)
//   * `dangerouslyAllowInsecureMainnetP2PK: true` is rejected on mainnet
//     (the compatibility contract is now fail-closed)
...
async function runErgoSafetyChecks(checks: ConformanceCheck[]): Promise<void> {
  ...
  // 4. dangerouslyAllowInsecureMainnetP2PK → INSECURE_MAINNET_MODE
  checks.push(
    await captureRejection({
      id: "L3.ergo.dangerously-allow-p2pk-rejected",
      description:
        "ergo-agent-pay rejects dangerous P2PK bypass flags on mainnet",
      expectedCode: "INSECURE_MAINNET_MODE",
      run: () =>
        ergoAssertSafety({
          operation: "issueNote",
  ...
  // 5. dangerouslyAllowUnauditedErgoTree → UNAUDITED_ERGOTREE
  checks.push(
    await captureRejection({
      id: "L3.ergo.dangerously-allow-unaudited-tree-rejected",
      description:
        "ergo-agent-pay rejects unaudited ErgoTree bypass flags on mainnet",
      expectedCode: "UNAUDITED_ERGOTREE",
      run: () =>
        ergoAssertSafety({
          operation: "issueNote",
  ...
  // 6. Testnet always allowed even with no script
  let testnetOk = false;
  try {
  ...
  // 7. verifyAuditedErgoTree({ requireMainnet: true }) refuses unsigned manifest
...
async function runBaseSafetyChecks(checks: ConformanceCheck[]): Promise<void> {
  ...
  // 2. Mainnet write with allow flag → UNAUDITED_CONTRACT
  checks.push(
    await captureRejection({
      id: "L3.base.dangerously-allow-rejected",
      description:
        "agentpay-base rejects unaudited contract bypass flags on mainnet",
      expectedCode: "UNAUDITED_CONTRACT",
      run: () =>
        baseAssertSafety({
          operation: "issueNote",
