...
//   npx accord-conformance sign --key-file ./private-key.txt result.json > signed.json
...
async function signCmd(argv: string[]): Promise<void> {
  ...
  if (!key) usageExit(`sign: --key-file <path> or --key 0x<hex> required`);
...
async function keygenCmd(): Promise<void> {
  ...
  process.stdout.write(
    `Store the private value in a local file outside git, sign with --key-file, and share '${publicKey}' so verifiers know it's you.\n`,
  );
  process.exit(0);
}
...
function printUsage(): void {
  process.stderr.write(
    [
      ...
      `  accord-conformance sign (--key-file <path> | --key <0xhex>) [--signer <id>] [--output <path>] <input.json>`,
