...
export function parseServerConfig(
  ...
  for (let i = 0; i < args.length; i++) {
    if (args[i] === "--address" && args[i + 1]) {
      ...
      if (val === "testnet" || val === "mainnet") {
        ...
        throw new Error(`--network must be "mainnet" or "testnet" (got "${val}")`);
      }
  ...
  if (!nodeUrl) {
    nodeUrl = env["ERGO_NODE_URL"] ?? NODE_URLS[network];
  }
...
const TOOLS = [
  ...
  },
  {
    ...
    description:
      "Build an unsigned EIP-12 transaction that creates a Reserve box (collateral backing a Note system). Returns JSON to sign. Refuses on mainnet without a compiled and audited 'script_ergo_tree'.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description:
      "Build an unsigned EIP-12 transaction that issues a Note (programmable bearer IOU) against a Reserve. Either provide 'task_hash' (hex BLAKE2b-256) or 'task_output' (the SDK will hash it). Refuses on mainnet without a compiled and audited 'script_ergo_tree'.",
    inputSchema: {
      type: "object",
      properties: {
