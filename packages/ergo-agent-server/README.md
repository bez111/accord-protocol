...
listens on `127.0.0.1` by default. Binding to any non-localhost interface
requires `--api-key`.
...
| `ERGO_ALLOW_INSECURE_DEV_MODE=1` | `--allow-insecure-dev-mode` legacy flag; no mainnet bypass |
...
When `--host` is not `127.0.0.1`, `localhost`, or `::1`, startup fails unless
`--api-key` / `ERGO_API_KEY` is set.
...
`script_ergo_tree`, the request returns `403 INSECURE_MAINNET_MODE`. The
legacy `--allow-insecure-dev-mode` flag is parsed for compatibility but does
not bypass mainnet safety.
