...
// There is deliberately no mainnet bypass.
//
 ...
 *   - mainnet + dangerous bypass flag: rejected with UNAUDITED_CONTRACT.
 *   - mainnet + no auditPolicy: rejected with UNAUDITED_CONTRACT.
 */
export async function assertProductionSafety(args: ProductionSafetyArgs): Promise<void> {
  if (args.network !== "base") return;

  if (args.dangerouslyAllowUnauditedContract === true) {
    throw new BaseAgentPayError(
      `Refusing to ${args.operation} on Base mainnet — unaudited contract ` +
        `bypass flags are disabled. Configure auditPolicy with a signed ` +
        `manifest entry marked mainnetAllowed: true.`,
      "UNAUDITED_CONTRACT"
    );
  }
  ...
  if (args.auditPolicy) {
    ...
    throw new BaseAgentPayError(
      `Refusing to ${args.operation} on Base mainnet — audit policy rejected the contract.\n` +
        ...
        `Supply a contract present in your externally signed audited manifest as ` +
        `mainnetAllowed: true.`,
      "UNAUDITED_CONTRACT"
    );
  }
  ...
  throw new BaseAgentPayError(
    `Refusing to ${args.operation} on Base mainnet — no auditPolicy is configured.\n` +
      `Mainnet writes require an audited contract bytecode hash. Configure\n` +
      `auditPolicy on the agent, typically backed by \`verifyAuditedContract\`\n` +
      `from agentpay-base with requireMainnet: true.`,
    "UNAUDITED_CONTRACT"
  );
}
