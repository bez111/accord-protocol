...
describe("assertProductionSafety — mainnet box-shape gate", () => {
  ...
  it("rejects dangerouslyAllowInsecureMainnetP2PK on mainnet", async () => {
    await assert.rejects(
      () =>
        assertProductionSafety({
          operation: "createReserve",
  ...
  it("rejects legacy allowInsecureDevMode on mainnet", async () => {
    await assert.rejects(
      () =>
        assertProductionSafety({
          operation: "createReserve",
...
describe("assertProductionSafety — mainnet audit gate", () => {
  ...
  it("rejects dangerouslyAllowUnauditedErgoTree on mainnet", async () => {
    await assert.rejects(
      () =>
        assertProductionSafety({
          operation: "issueNote",
  ...
  it("rejects per-call unaudited-tree override on mainnet", async () => {
    await assert.rejects(
      () =>
        assertProductionSafety({
          operation: "issueNote",
